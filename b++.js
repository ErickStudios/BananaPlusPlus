...
function SyntaxSolve(Syntaxa, Env)
{
    ...
    else if (Syntax.startsWith("JavaScript[") && Syntax.endsWith("]"))
    {
        ...
        Env.locals.forEach((value, key, map) => {
            header += "exports_bpp.set(" + EcodeJsStr(key) + ", " + EcodeJsStr(value) + ");" + "\n";
        });
        Env.constants.forEach((value, key, map) => {
            header += "exports_bpp.set(" + EcodeJsStr(key) + ", " + EcodeJsStr(value) + ");" + "\n";
        });
        Env.variables.forEach((value, key, map) => {
            header += "exports_bpp.set(" + EcodeJsStr(key) + ", " + EcodeJsStr(value) + ");" + "\n";
        });
...
if (onlyLanguaje == false) {
    ...
    {
        let ref_val = StringScape(SyntaxSolve(ref, envi));
        let num_resalt = StringScape("\\k33m");
        let string_resalt = StringScape("\\k32m");
        let function_resalt = StringScape("\\k36m");
        let resalt_normal = StringScape("\\k0m");
        let IsClassBody = SyntaxSolve(ref + ".new", envi) != (ref + ".new");
        let DarkColor = StringScape("\\k2m");
        let non_function = [32];

        // si es una clase
        if (IsClassBody) { return function_resalt + "[class " + ref + "]" + resalt_normal;}
        // si es una funcion
        else if (Array.isArray(GetParams(ref_val)) && GetParams(ref_val).length > 0) { return function_resalt + "[Function: " + ref + "]" + resalt_normal; }
        // si es un undefined
        else if (ref_val == "") { return DarkColor + StringScape("\\k37m") + "undefined" + resalt_normal;}
        ...
        // un string
        else if (isNaN(Number(ref_val))) { return string_resalt + EcodeJsStr(ref_val) + resalt_normal; }
        // un numero
    ...
    function cli() {
        ...
        rl.question('> ', (respuestaa) => {
            ...
            else if (respuesta == ".mem")
            {
                ...
                {
                    let variable = SyntaxSolve(respuesta_anterior, envi);
                    ...
                    console.log(StringScape("\\k32m") + EcodeJsStr(donde_esta) + StringScape("\\k0m"));
                }
                else if (respuesta == "we")
                {
                    console.log(StringScape("\\k32m") + EcodeJsStr("que") + StringScape("\\k0m"));
                }
                else if (respuesta_anterior == "we")
                {
                    switch (respuesta) {
                        case "ejecuta algo":
                            console.log(StringScape("\\k32m") + EcodeJsStr("pero pon que ejecutare, ahhh") + StringScape("\\k0m"));
                            break;
                    
                        default:
                            console.log(StringScape("\\k32m") + EcodeJsStr("q?") + StringScape("\\k0m"));
                            break;
                    }
                }
            }
    ...
    if (use_stdlib == true) {  
        ...
        if (fs.existsSync(stdlibPath)) {
            envi = ExCode(fs.readFileSync(stdlibPath, 'utf8'), envi);
        }
    ...
    if (files_to_execute.length == 0) {
        ...
        if (SyntaxSolve("EnvContains[\"stdlib_ver\"]", envi) == "true")
        {
            console.log("la version de la stdlib que se esta ejecutando es la " + SyntaxSolve("stdlib_ver", envi))
        }
        cli();
        }
    else exit(0);
