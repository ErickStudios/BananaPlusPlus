/**
 * este es el archivo de el lenguaje de programacion, debido a su modalirad puede tambien ser
 * usado en la extension de vscode
 */
const fs = require("fs");
 ...
 * @returns {Promise<string>} la respuesta
 */
function readl(ask) {
    ...
    for (let index = 0; index < str.length; index++) {
        ...
        switch (element) {
            ...
            default:
                ...
                // si no es imprimible añadir el codigo
                if (code < 32 || code > 126) stra += "\\u" + code.toString(16).padStart(4, '0'); 
                // si lo es añadir el caracter
                else stra += element;
        }
    }
    // retornar la codificacion
    return "\"" + stra + "\"";
}
/**
 * StringScape
 ...
 */
function SyntaxSolve(Syntaxa, Env)
{
    // convertirlo a string
    let Syntax = String(Syntaxa);
    // x
    ...
    // x==y
    else if (Syntax.split("==").length == 2) return String(SyntaxSolve(Syntax.split("==")[0], Env) == SyntaxSolve(Syntax.split("==")[1], Env))
    // x!=y
    else if (Syntax.split("!=").length == 2) return String(SyntaxSolve(Syntax.split("!=")[0], Env) != SyntaxSolve(Syntax.split("!=")[1], Env))
    // x<y
    else if (Syntax.split("<").length == 2) return String(Number(SyntaxSolve(Syntax.split("<")[0], Env)) < Number(SyntaxSolve(Syntax.split("<")[1], Env)))
    // x>y
    else if (Syntax.split(">").length == 2) return String(Number(SyntaxSolve(Syntax.split(">")[0], Env)) > Number(SyntaxSolve(Syntax.split(">")[1], Env)))
    // JavaScript[x] ejecuta codigo javascript
    else if (Syntax.startsWith("JavaScript[") && Syntax.endsWith("]"))
    {
        // el codigo
        let code = SyntaxSolve(Syntax.substring(11, Syntax.length - 1), Env);
        // encabezado
        let header = "//autogenerado para exportacion de banana++\n";
        // mapa
        header += "var exports_bpp = new Map();\n";
        // exportar variables locales (solo lectura)
        Env.locals.forEach((value, key, map) => { header += "exports_bpp.set(" + EcodeJsStr(key) + ", " + EcodeJsStr(value) + ");" + "\n"; });
        // exportar constantes (solo lectura)
        Env.constants.forEach((value, key, map) => {header += "exports_bpp.set(" + EcodeJsStr(key) + ", " + EcodeJsStr(value) + ");" + "\n";});
        // exportar variables globales (solo lectura)
        Env.variables.forEach((value, key, map) => {header += "exports_bpp.set(" + EcodeJsStr(key) + ", " + EcodeJsStr(value) + ");" + "\n";});
        // añadir encabezado
    ...
    // Sys.Read[]
        ...
        // preguntar algo
        let ask = SyntaxSolve(Syntax.substring(9, Syntax.length - 1), Env);
        // completado'
        let completed = false;
        // respuesta
        let answare = "";
        // esperar respuesta
        (readline.createInterface({input: process.stdin, output: process.stdout})).question(ask, (respuesta) => {answare = respuesta; completed = true; });
        // si se completo
        while (completed == false) { for (let index = 0; index < 1000; index++) {}};
        // retornar respuesta
        return answare;
    }
    // milisegudos
 ...
 */
function NormalizePath(directory) { return directory.replaceAll("/", path.sep);}
/**
 * IsOperator
 * 
 ...
 * @returns si es uno
 */
function IsOperator(char) { return char == '$'||  char == '^' || char == '*' || char == '+' || char == '-' || char == '/' || char == '%' || char == '√';}
/**
 * VarSyntax
 * 
...
function VarSyntax(VarName, Env)
{
    ...
    let retval = VarName;
    // variables globales
    Env.variables.forEach((val, key, map) => {retval = retval.replaceAll(`@get[${key}]`, val);});
    // variables locales
    Env.locals.forEach((val, key, map) => {retval = retval.replaceAll(`@get[${key}]`, val);})
    // constantes
    Env.constants.forEach((val, key, map) => {retval = retval.replaceAll(`@get[${key}]`, val);})
    // retornarlo
    return retval;
}
/**
 ...
 */
function IsWhileSpace(str) { if (str == ' ' || str == '\t' || str == '\n' || str == '\r') return true; return false;}
/**
 * GetParamsStrict
 * 
 * obtiene los parametros estrictos
 * @param {string} func el cuerpo
 * @returns {string[]} los parametros
 */
function GetParamsStrict(func)
{
    // los parametros
    let pars = "";
    // recorrerlo
    for (let index = 1; index < func.length; index++) { const element = func[index]; if (element == ')') break; else pars += element;}
    // separalos
    let psm = pars.split(",");
    // crear array
    let nm = [];
    // recorrerlos
    psm.forEach(element => {let el = element.trim();nm.push(el);});
    // retornarlo
    return nm;
}
/**
...
{
    // si no incluye
    if ((func.includes('(', 0) && func.includes(')', 0)) == false) return new Array().push(32);
    // parametros
    let pars = "";
    // ponerlos
    for (let index = 1; index < func.length; index++) {
        ...
        // si se cierra
        if (element == ')') break; else pars += element;
    }
    // separalos
    let psm = pars.split(",");
    // el array
 ...
 * @returns si es
 */
function IsAValidCharForWord(element) { return (IsWhileSpace(element) == false && element != '&' && element != '|' && element != '+' && element != '$' && element != '-' && element != '=' && element != '/' && element != '*' && element != '(' && element != ')' && element != '{' && element != '}' && element != ':' && element != ';');}
/**
 * ExCode
 * 
 * ejecuta un codigo
        ...
        else
        {
                    ...
                    filestoimport.forEach(elemental => {
                        ...
                        if ((SyntaxSolve(elemental, EnvNew)).endsWith(".b++")) {
                            ...
                            if (OnlyDocumentationQuiet == false) {
                                // el archivo
                                elfile = path.join(SyntaxSolve("Sys.RunPath", EnvNew), NormalizePath(SyntaxSolve(elemental, EnvNew)) + ".b++");
                                // si existe
                            ...
                            else
                            {
                                ...
                                console.log(elfile);
                                // verificar si existe
                                if (fs.existsSync(NormalizePath(elfile))) { 
                                    let file_content = fs.readFileSync(NormalizePath(elfile) , 'utf-8'); EnvNew = ExCode(file_content, EnvNew);
                                }
                            }
                        }
                    });
            ...
            else
            {
                ...
                {          
                    ...
                    // solo en este stack
                    else { Env.locals.set(Word, FunctionBody); VarsThatDeletes.push(Word); }
                    // no se pondra
                    PutInOut = false;
                }
                ...
                else if (SysGetChar) { SysGetChar = false; let Str = SyntaxSolve("Sys.__str__", EnvNew); let CharGet = Number(SyntaxSolve("Sys.__char__",EnvNew)); EnvNew.variables.set(Word, Str[CharGet]); }
